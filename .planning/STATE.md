...
- **PHASE 5 — cancel/uncancel and edit guards (REQ-024…REQ-028 / GitHub issue #15). Code complete
  and verified against a live local stack, 2026-09-22. Awaiting the user's browser pass.** Baseline
  `d7d0a2c`, tree uncommitted, on branch
  `franklinnevesfilho/cancel-uncancel-and-edit-guard-bugs-can-silently`. Three tasks dispatched as
  one parallel block on disjoint files (migration + `database.ts`; `appointments.ts`; the two admin
  pages), then a `/code-review high` round and one fix track. Verifier round 1 (full E1–E4 + E3b +
  S1–S3, including an executed and clean DOWN path) and round 2 (scoped re-check of the review
  fixes) both found **no defect**; round 1's BLOCKED was evidence-only. Changed: new migration
  `supabase/migrations/20260922120000_appointments_status_before_cancel.sql` (additive
  `appointments.status_before_cancel text NULL`, no backfill, two named CHECKs; DOWN executed and
  verified); `src/types/database.ts:309,327,345`; `src/lib/actions/appointments.ts` (edit guard
  extended to `cancelled`; marker written on both cancel paths; `cancelAppointment` refuses
  `completed` and already-`cancelled`, compare-and-set; `uncancelAppointment` restores
  `status_before_cancel ?? 'scheduled'`; the `appointment_employees` delete-and-reinsert replaced by
  an id-preserving reconcile at `:766-816`); both admin appointment pages. Drift gate: **REQ-028
  added after implementation (AMD-5 / D-42)** for the `updateAppointment` compare-and-set the code
  review found missing — see `05-SUMMARY.md` Deviations. **Next action: the user runs Half B, the
  12-step browser script in `05-02-PLAN.md`; the local DB is already reset, seeded and holding the
  stamped row `f0db1f08-5dcc-459c-9cc3-80aa5c242973` (`clocked_in_at` set, `admin_notes='keep me'`)
  as its precondition — the user needs only `npm run dev` and to sign in. The user must also answer
  OQ-C3: a mistakenly-completed appointment now has no escape hatch.** Deploy order (D-40): the
  migration ships first or in the same release as the code — never the code alone.
- **PHASE 4 — void releases appointments (REQ-022 / GitHub issue #18). Code complete and verified
  against a live local stack, 2026-09-21. Awaiting the user's browser pass.** Baseline `926f088`,
  tree uncommitted. Four tasks dispatched as one parallel block, then a `/code-review high` round
  and two fix tracks. Verifier round 1 (full E1–E6) and round 2 (scoped re-check of the review
  amendments) both PASS, no drift. Changed: new migration
  `supabase/migrations/20260921130000_void_releases_appointments.sql` (partial unique index
  `invoice_appointments_live_appointment_id_idx ... WHERE (is_archived = false)` replacing the
  table-wide `invoice_appointments_appointment_id_key`; DOWN executed and verified);
  `src/types/database.ts:512`; `src/lib/actions/invoices.ts` (`voidInvoice` release + cache clear,
  plus compare-and-set status guards on `voidInvoice`/`issueInvoice`/`markInvoicePaid`);
  `supabase/checks/pricing_backfill_check.sql` (both cache invariants narrowed to live rows
  together, D-25); both invoice builder pages. **Next action: the user runs Half B, the 18-step
  browser script in `04-02-PLAN.md`, then `pricing_backfill_check.sql` §3 on the resulting state.**
  Deploy order (§7.0b, D-33): the migration ships first or in the same release as the code — never
  the code alone.
- **Out-of-milestone UI work, 2026-09-21 — jobs list/card interaction refactor.** Tier 1, not part
  ...
  acceptance criteria. `/code-review high`: no findings in either changed file. **Reconciled
  2026-09-21: this is no longer uncommitted.** It landed via PR #19 and is in history at `26e5018`
  / `b0096cb`, merged as `926f088`, which is now HEAD. The browser pass of `/solutions/jobs` on
  desktop and a small viewport is still owed by the user.
...
| **D-26** | **2026-09-21 — REQ-022 is SCHEDULED as Phase 4 (GitHub issue #18), on the user's instruction.** Two plans, four tasks, all four concurrent: `04-01-T1` migration + `database.ts`, `04-01-T2` check script, `04-02-T1` `voidInvoice`, `04-02-T2` both builders. **D-23 is not overturned.** D-23 held that clearing the cache alone is a false affordance *because the appointment stays unbillable*; Phase 4 removes that premise, which is exactly what OQ-R1 option (c) named. **D-24 survives unchanged** and is a hard boundary on `04-02-T1`. | This is the outcome the planner recommended in OQ-R1 ("(a) now, (c) when there is room"), and the user has now made room. Scheduling it does not reopen any settled decision: the `02-03` reverts stand, the tree is at the clean pre-REQ-022 baseline, and `04-02-T1` adds the release *and* the clear together rather than the clear alone. |
| **D-27** | **AMD-3, 2026-09-21 — REQ-002's two cache invariants key on `invoice_appointments.is_archived` alone. No join to `invoices`, no `status` predicate.** REQ-022's acceptance had said Branch B *"additionally"* excluding released rows, i.e. both predicates. The status predicate is dropped. Both invariants still move together (D-25 unchanged). **AMD-3b:** both labels gain the word *live*, overriding Branch B's "labels unchanged" clause for these two rows. | The two predicates agree on every row the application produces after Phase 4 — `voidInvoice` is the only writer of the marker and it writes it on exactly the rows whose invoice it voided. They disagree only on invoices voided *before* the migration, which hold live junction rows backed by correct caches: a status predicate reports those as a false "MUST BE 0" (the first invariant skips them on status, the second finds no "live" row and counts them), while `is_archived` alone reads them as consistent. So the `is_archived`-only wording is correct under **either** answer to OQ-V1, and the two-predicate wording only under one — which also decouples `04-01-T2` from that open question entirely. A label still reading "its junction row" would assert something the narrowed query no longer checks, and this file is read by a human hunting a number that must be 0. |
| **D-28** | **AMD-4, 2026-09-21 — REQ-018's acceptance bullet is split.** *Issuing, archiving and restoring* still leave `appointments.billed_price_cents` untouched; *voiding* now clears it, for exactly the appointments it releases. REQ-018's closing sentence "Until REQ-022 lands, `voidInvoice` writes `billed_price_cents` never" has expired by its own terms. Superseded text preserved in place. | REQ-022 is landing, which is the condition that sentence named. The archive/restore half is D-24 and is untouched — it is the one clause a well-meaning implementer is most likely to "improve" while in that file, so it is restated as an explicit out-of-scope item in `04-02-PLAN.md`. |
| **D-29** | **2026-09-21 — issue #18's "partial unique index scoped to non-void invoices" is delivered via the `is_archived` release marker, not an invoice-status predicate. REQ-022(b) is confirmed as the technically correct reading.** | A partial index predicate may only reference columns of the table it indexes; it cannot join to `invoices`, and `invoice_appointments` has no status column. The two formulations are equivalent in behaviour precisely because `voidInvoice` is the only thing that sets the marker. This is a clarification of *how* #18 is delivered, not a change to the agreed mechanism — recorded so a reviewer reading #18 against the migration does not read the difference as drift. The column is already the right shape: `is_archived boolean DEFAULT false NOT NULL` (`20260918120000:142-145`); NOT NULL is load-bearing, since a nullable flag would leave rows outside both halves of the predicate and silently exempt from uniqueness. |
| **D-30** | **2026-09-21 — `voidInvoice` writes in the order status → release → clear, in three non-atomic round trips. No RPC, no transaction wrapper.** | The ordering is the safety argument and reads backwards, so it is commented in the code. If the release write fails after the status write lands, the invoice is void and its appointments are still consumed — today's behaviour, a state the product has always had. Releasing *first* would mean a failed status write leaves a draft or issued invoice still displaying and totalling lines whose appointments are already back in the billable pool: a double-billing window this phase would have invented. A `SECURITY DEFINER` function doing all three atomically was considered and rejected under constitution §12 (existing conventions — `createInvoice` already has a non-atomic invoice-then-links path with a hand-rolled restore) and §13 (no premature abstraction). Recorded so it is not re-proposed as a review finding. |
| **D-32** | **2026-09-21, plan-check remediation — the local Supabase database is a shared mutable resource in Phase 4, and only the `verifier` touches it.** The four coders run **static gates only**: diff inspection, grep, `npm run lint`, `npx tsc --noEmit`, plus `npm run build` for `04-02-T2`. No coder runs `supabase start`, `db reset`, `db query`, `seed:admin`, `test-data.sql` or `npm run dev`. Every DB-state acceptance item is marked **[verifier]** or **[user]** and executed once, serially, after all four tasks land. **The DOWN path (E6) is the verifier's**, so `04-01-T1` writes its header with the precedent's "NOT yet executed" wording and the verifier — which cannot edit files — reports the header line to correct on success, for the orchestrator to dispatch as the phase's last one-line edit. | File exclusivity made the four tasks concurrent; it does not make their *verification* concurrent. `04-01-T1`'s counts (`released rows after reset = 0`, `cached prices = 0`) hold only on a freshly reset database; `04-01-T2`'s discriminating check needs fixture rows; `04-02-T1`'s needs a real void. Run at the same time, any `db reset` erases another task's fixtures and any fixture falsifies the "want 0" counts — the results become order-dependent, which is another way of saying they are not evidence. This costs no parallelism: it moves checks out of four coders into one pass that had to happen anyway. |
| **D-33** | **2026-09-21 — deploy order for Phase 4 is not free: the migration ships FIRST or in the SAME release as the application code, never the code alone.** Recorded in `04-CONTEXT.md §7.0b`, in `04-02-PLAN.md`'s header, and to be carried into the phase summary handed to the user. | Migration-then-code is safe — the partial index is in force and nothing writes `is_archived`, so behaviour is identical to today until the code lands. Code-then-migration is **exactly the half-fix REQ-022 forbids**: `voidInvoice` would release rows and clear caches while the table-wide UNIQUE still stands, so both builders would offer the visit again and `createInvoice` would reject it with the generic "already attached to another invoice" error — a false affordance with no visible cause, worse than the bug. Deployment is a human step outside this plan (constitution §5), so nothing enforces this but writing it down. |
| **D-34** | **2026-09-21 — the Phase 4 verifier verdict is PASS or BLOCKED, with no third state.** Until the user's browser pass is reported back, the phase-level verdict is **BLOCKED** and the orchestrator's next action is to relay the 18-step script, not to close the phase. Earlier drafts of `04-01-PLAN` and `04-CONTEXT` said "report as verified, phase not closed"; that wording is removed. | The verifier's contract in this project is a binary gate. A third state has no defined next action, so an orchestrator receiving it either invents one or stalls. And the substance is right: steps 7–10 and 16–18 of the browser pass are the only *executed* proof of REQ-022's acceptance, so a phase closed without them would be closed on inference. |
| **D-35** | **2026-09-21 — the migration's `DROP CONSTRAINT` is classified in the header as a §4 widening, explicitly, with REQ-022(b) cited.** No ARCH-1-style human-approved exception is needed. | Every row the old table-wide UNIQUE permitted is still permitted by the partial index; the permitted set strictly grows (one live claim *plus* any number of released records); no column or row is touched and no data becomes unreachable. The planner and the plan-check reviewer agree, but the migration never *said* so, which invites the question a second time at review — and the precedent migration `20260921120000` says the equivalent about its own drop. |
| **D-31** | **2026-09-21 — Phase 4's four tasks have no dependency edges between them; the migration does **not** gate the action or the builder queries.** | `invoice_appointments.is_archived` has existed since `20260918120000:142-145` and is already typed in `src/types/database.ts:487,497,507`, so every query and update `04-02` writes compiles, type-checks and executes against the tree at `926f088`. They simply have no *effect* until the index lands. Only verification needs all four, and that is a gate property. "Migration and types first, then queries" is habit here, not a dependency, and serialising on it would turn a one-block phase into a four-block one for nothing. |
| **D-36** | **2026-09-22, Phase 5 — restoring the status a cancellation replaced needs a persisted column. `appointments.status_before_cancel text NULL`, additive, no backfill, two CHECK constraints (value domain `scheduled\|in_progress\|completed`, and non-NULL only while `status = 'cancelled'`).** NULL means *not recorded* and restores to `'scheduled'` — today's behaviour, and what every pre-existing cancelled row keeps. The domain CHECK **permits `'completed'`** although D-37 makes that value unreachable through the application. | It is not derivable. Completion is an explicit admin status write (`appointment-form.tsx:320` → `appointments.ts:722`); an appointment can be completed with no crew assigned at all; and `employee_clock` (`20260917120000:72-113`) has **no** status predicate, so an employee can clock out on an appointment nobody marked completed. Clock times are therefore not a proxy for status, and deriving would reintroduce the same silent loss one layer deeper. `in_progress` needs the column as much as `completed` does: even with D-37 in force, *in_progress → cancel → reopen* would otherwise land on `scheduled` and discard a distinction the admin made. Permitting `'completed'` in the CHECK keeps an application policy out of the schema — if the product later allows cancelling a completed appointment, the code changes and the schema does not. The migration header says so, so a reviewer does not read the gap as an oversight. |
| **D-37** | **2026-09-22, Phase 5 — `cancelAppointment` HARD-REFUSES a completed appointment; it does not record-and-restore. The detail page HIDES the Cancel control for `completed` — hidden, not disabled, and with no error text.** A second refusal branch covers an already-cancelled row. Both writes become compare-and-set on the status read a moment earlier. | Three reasons in order of weight. (1) `completed` is **already terminal** in this product at two layers (`appointments.ts:648`, `edit/page.tsx:178-181`); a cancel path that silently overrides it contradicts a guard 250 lines away in the same file. (2) The complaint in issue #15 is *silence*, and record-and-restore is a silent state machine — the admin would see `CANCELLED` while the system privately remembered `completed`, discoverable only by reading the column. Refusal is visible at the moment of the mistake. (3) Refusal invents no invoice semantics: a completed appointment is the one most likely to be invoiced, and this application has no notion of a cancelled-but-billed visit. **Hidden rather than error-reported** because the detail page's two `'use server'` wrappers (`appointments/[id]/page.tsx:157-167`) discard the action's return value, so an error would be invisible and a disabled button would read as broken; the `COMPLETED` badge at `:190-196` is the explanation. The server guard stays regardless, to stop the write from a stale page. **The compare-and-set is load-bearing, not ceremony**: without it, cancelling in one tab while another marks the row complete overwrites the completion and records `status_before_cancel = 'scheduled'` — the exact bug, through the back door. |
| **D-38** | **2026-09-22, Phase 5 — a cancelled appointment is BLOCKED from editing**, at both layers, in the same shape as the existing `completed` guard, with its own message naming the remedy. | The reopen path is lossless under D-36, so *Reopen → edit → cancel again* costs one click and loses nothing. It also collapses the state machine: `cancelled` then has exactly **one** exit, so the form's status `<select>` can no longer move a row out of `cancelled` and the marker needs clearing on exactly one path. Leaving cancelled rows editable would have made the `<select>` a third write path for the marker. And it mirrors an idiom already in the codebase rather than inventing one. |
| **D-39** | **2026-09-22, Phase 5 — the `appointment_employees` delete-and-reinsert is replaced by a reconcile REGARDLESS of D-38, because it is a live data-loss bug independent of cancellation.** Surviving rows are **never written at all**: keep every row whose `employee_id` is still submitted, delete only removed ones by `id`, insert only genuinely new ones. No `is_archived` filter on the read. Duplicate rows for one employee are all kept or all deleted, never collapsed. **No helper is extracted and no unit test is added.** | `updateAppointment:739-764` fires on **every** non-`series` edit, so an employee clocks in, an admin fixes the end time, and the hours are gone — on an ordinary `scheduled` appointment. Blocking cancelled edits does nothing for that. It also changes the row `id`, which is the key both `updateClockTime` (`:950-977`) and the `employee_clock` RPC address, so the damage is identity, not just values. **No `is_archived` filter** because the delete it replaces has none: filtering would strand an archived row for a removed employee and duplicate an employee who has one. **No de-duplication** because `appointment_employees` has no unique constraint on `(appointment_id, employee_id)` (`20260427000000:125-135`, and the non-idempotent seed found in Phase 1 proves duplicates occur) — collapsing them would be a silent delete of clock data, i.e. the bug. **No helper** under constitution §13 (no second caller) and §15 (the testing bar is money and resolution functions), and because `appointments.ts` carries `'use server'`, where every export must be an async function — a pure exported `reconcile()` there is a build error, and a new module for one caller is the abstraction §13 forbids. |
| **D-40** | **2026-09-22, Phase 5 — deploy order: the migration ships FIRST or in the SAME release as the application code, never the code alone.** Recorded in `05-CONTEXT.md §8`, in `05-02-PLAN.md`'s header, and to be carried into the phase summary handed to the user. | Migration-first is safe: the column exists, nothing writes it, behaviour is identical to today. Code-first is worse than the bug — `cancelAppointment` would send `status_before_cancel` to a table without that column, PostgREST answers `400 / 42703`, and the detail page's wrapper discards the result, so **Cancel silently does nothing at all**. Same shape as D-33, and the same reason it had to be written down: deployment is a human step outside this plan (constitution §5). |
| **D-42** | **AMD-5, 2026-09-22, Phase 5 — drift adjudication: `updateAppointment`'s compare-and-set is CORRECT and the SPEC was short. A new requirement, REQ-028, is added to describe it; REQ-024 and REQ-025 are NOT widened.** What shipped: `.eq('status', existingAppointment.status)` on `updateAppointment`'s appointment update (`src/lib/actions/appointments.ts:734`), plus a re-read of the row in the zero-row branch (`:744-763`) so a lost update reports *"The appointment changed while you were editing it. Refresh and try again."* instead of *"Appointment not found."* The verifier's finding — that REQ-024's compare-and-set bullet is scoped to `cancelAppointment` and REQ-025 asks only that the marker be *written* on both paths — is accepted as accurate; the gap was in the plan, not the code. `05-02-PLAN.md` Change 2 is amended into Parts A (original) and B (the guard), with the original text preserved. | The review found a **live** lost-update, not a hypothetical: the status is read at `:633-638` and written many statements later, so a completion landing in another tab in between is overwritten, and an edit selecting *Cancelled* then records `status_before_cancel = 'scheduled'` — a destroyed completion **and** a marker asserting a falsehood. That is the identical silent loss REQ-024/REQ-025 exist to close, reached through the second cancel door (`05-CONTEXT.md §2.9`). Shipping the guard and leaving the spec silent would have left the most important line in the phase attributable to no requirement. **A new REQ rather than a widening** because what shipped is broader than either requirement would honestly describe: the form always submits a `status`, so the guard protects an edit that only changes the notes field, not merely a cancellation. Folding it into REQ-024 ("a cancellation never overwrites a completed appointment") would misname it; folding it into REQ-025 would make a column-contents requirement carry a concurrency rule. A separate ID also keeps the audit legible — REQ-024 and REQ-025 were satisfied as written. |
| **D-43** | **AMD-5, 2026-09-22, Phase 5 — wording only, no requirement affected. `cancelAppointment`'s completed-refusal now reads `'A completed appointment cannot be cancelled. Completed is a final status.'`** The plan had specified *"…Change its status first if this was recorded in error."* | The review found that sentence names a remedy the UI forbids: the edit page refuses a `completed` appointment (`edit/page.tsx:178-181`), so an admin cannot change its status. Telling them to do the impossible is worse than saying nothing. `05-02-PLAN.md` Change 3's quoted string is corrected; no REQ quoted it. **The wider problem the message was reaching for is real and is now OQ-C3** — there is no escape hatch at all for a mistakenly-completed appointment, and that is the user's call, not a wording fix. |
| **D-41** | **2026-09-22, Phase 5 — D-32 is carried forward verbatim. The local database is a shared mutable resource and only the `verifier` touches it.** All three coders run static gates only: diff, grep, `npm run lint`, `npx tsc --noEmit`, `npm test`, `npm run build`. No coder runs `supabase start`, `db reset`, `db query`, `seed:admin`, `test-data.sql` or `npm run dev`. The DOWN path is the verifier's, so `05-01-T1` ships its header with the "NOT yet executed" wording and the verifier reports the header line to correct. | Identical reasoning to D-32: `05-01`'s E2 count (`status_before_cancel IS NOT NULL` = 0) holds only on a freshly reset database, while `05-02`'s S3 needs a stamped fixture row. Run concurrently, each falsifies the other and the results become order-dependent — which is another way of saying they are not evidence. It costs no parallelism; it moves checks into one pass that had to happen anyway. |
...
| **AS-12** | **Phase 4, 2026-09-21.** `voidInvoice` will be the **first code path in this application that ever runs an `UPDATE` against `invoice_appointments`** — every existing write path deletes and re-inserts. That table has carried a `set_updated_at` BEFORE UPDATE trigger since the schema snapshot, which raised `record "new" has no field "updated_at"` on every UPDATE until `20260918120000` added the column (D-06, B-01). The column exists now, so the assumption is that the UPDATE simply works. | `04-02-T1` fails at runtime with that exact error, and the browser smoke script's step 6 (Void) 500s. The plan requires the implementer to verify it empirically rather than assume, and the smoke script flags step 6 as where it would surface. Remediation would be a one-line migration, not a replan. |
| **AS-13** | **Phase 4, 2026-09-21.** The count of invoices already voided on the live database (`blhxzilsjuzbeoxtkbap`) is **unknown** — a consequence of AS-09. Under OQ-V1's forward-only default, each one of them keeps its appointments permanently unbillable after Phase 4 ships. | If that count is large, the forward-only default satisfies issue #18's letter and misses its point for the rows that actually provoked it. This is why `04-01-T1` ships the sizing query in the migration header: the number is obtainable in one read before deployment, and the remediation is two `UPDATE`s away. Answering OQ-V1 "backfill" changes `04-01-T1` only, and needs a named constitution §7 exception. |
...
| **AS-14** | **Phase 5, 2026-09-22.** No production row currently sits at `status = 'cancelled'` having previously been `completed` — or if any does, nobody can tell, because nothing recorded it (AS-09: the planner has never seen the live data). The forward fix restores such rows to `'scheduled'` on reopen, exactly as today. | Nothing changes and nothing breaks: NULL → `'scheduled'` **is** today's behaviour, so the phase is strictly an improvement on every legacy row. There is no backfill to authorise and no constitution §7 question to answer — unlike Phase 4's AS-13, this one needs no decision from the user. Recorded only so nobody proposes a backfill that would have to invent data. |
| **AS-15** | **Phase 5, 2026-09-22.** `appointment_employees.is_archived` is never set to `true` by any code path in the application — grep over `src/` finds no writer. The reconcile therefore operates on a table where the flag is uniformly `false` in practice, and reads it without filtering to match the delete it replaces. | If some path does set it (a future soft-delete, or a hand-run SQL fix), the reconcile still behaves correctly by construction: an archived row for a surviving employee is preserved untouched, and one for a removed employee is deleted along with their live rows. That is the intended semantics either way, which is why the plan specifies *no filter* rather than *filter on false*. |

---

## Open questions — PHASE 5 (open; defaults applied, nothing is blocked)

> OQ-C1 and OQ-C2 are relayed to the user by the orchestrator. **Neither stops dispatch.** Every
> Phase 5 task is written against the stated default and can be executed today.
>
> **OQ-C3 is different and is the one thing the user must actually rule on.** It was opened
> 2026-09-22, *after* the code shipped and verified, by the post-implementation `/code-review high`.
> It has **no default applied and no fix designed** — deliberately. It does not block the phase's
> verification; it is a product decision about whether the phase should ship as it stands.

| # | Question | Planner's recommendation | What rides on it |
|---|---|---|---|
| **OQ-C3** | **A mistakenly-completed appointment now has no escape hatch at all. Is that acceptable, and if not, what replaces it?** After this phase, an admin who picks *Completed* by mistake can no longer edit the appointment's date, price, notes or crew (the pre-existing `completed` edit guard), **cannot cancel it** (D-37 refuses, and the detail page does not render the Cancel control), and **cannot reopen it** (Reopen only applies to `cancelled`). Before this phase, *Cancel → Reopen* was that escape hatch — it reset the row to `scheduled`. This work closes it without adding a replacement, and a wrongly-completed appointment **stays billable on the next invoice**. | **None — this is the user's decision and the planner is deliberately not making it.** The tradeoff is stated, not resolved: D-37 bought a single, honest rule about what `completed` means (one terminal status, enforced at both layers, no silent state machine, no cancelled-but-billed invoice question) and the price is that a fat-fingered *Completed* is now unrecoverable through the UI. Both halves are real. What the planner will say is that the cost is **bounded and reversible** — it is a UI affordance, not a data loss; the row is intact and correct in the database — and that the three obvious remedies (an admin-only "Undo completion" control; allowing `completed → cancelled` after all, i.e. OQ-C1 answered (b); or allowing an edit that changes *only* the status away from `completed`) differ mostly in how much invoice semantics they drag in. | **Nothing in the shipped code, unless the user wants it changed.** Phase 5's verification stands either way. If the user wants a remedy it is **new work, its own requirement and its own phase**, not a Phase 5 task: the cheapest option (an "Undo completion" action + control, guarded to refuse when the appointment holds a live `invoice_appointments` row) is one action, one button and one requirement. Answering it as *"accept it"* costs nothing and closes the question. **No guard is touched and no fix is designed until the user rules.** |
| **OQ-C1** | **When an admin clicks Cancel on an appointment that is already marked Completed, should the system (a) refuse — the Cancel button is not offered for a completed visit at all — or (b) allow it, quietly remember that it was completed, and put it back to Completed if the visit is reopened?** Today it allows the cancel and then loses the completion entirely on reopen, which is the bug. | **(a) refuse.** Reasoning at D-37: `completed` is already terminal for editing in this product, so refusing keeps one rule instead of two; the complaint in the issue is *silence*, and (b) is a silent state machine the admin cannot see; and (b) opens a question this phase cannot answer — a completed visit is the one most likely to be invoiced, and there is no such thing as a cancelled-but-billed visit here. | **`05-02` only.** `05-01-T1` is unaffected either way — the column already permits `'completed'` by design. Answering **(b)** changes `05-02-T1`'s guard (the refusal becomes a marker write) and `05-02-T2`'s control (Cancel is rendered again for completed, and the detail page needs a visible "previously completed" affordance so the state is not secret). One block of rework on two tasks, not a replan. |
| **OQ-C2** | Should the Reopen button name the status it will restore ("Reopen as In Progress")? | **No.** The restored status appears on the badge a second later, and the label needs the marker plumbed into the page query plus a fallback string for legacy NULL rows — new surface for a decision the admin does not have to make. | `05-02-T2` only, and only additively. |

---

## Open questions — PHASE 4 (open; defaults applied, nothing is blocked)

> Both are relayed to the user by the orchestrator. **Neither stops dispatch.** Every Phase 4 task
> is written against the stated default and can be executed today; a different answer changes
> `04-01-T1` and nothing else.

| # | Question | Planner's recommendation | What rides on it |
|---|---|---|---|
| **OQ-V1** | **Should the migration also release the appointments on invoices that were voided *before* it ran?** Every invoice already voided on the live database holds junction rows marked live and appointments whose billed amount is still cached. Nothing in the forward fix touches them, so **those visits stay permanently unbillable even after issue #18 ships** — the exact complaint, for the exact rows that provoked it. Releasing them means also clearing `appointments.billed_price_cents` on them, because the two must agree. | **Forward-only, by default — and the reason is authority, not mechanism.** It is *not* that clearing `appointments.billed_price_cents` is inherently a §7 violation: `voidInvoice` clears exactly that column at runtime, and that is REQ-022(d), which the user approved. The column is a documented display cache and the amount survives on `invoice_appointments.billed_amount_cents` either way. The distinction is that REQ-022(d) is **approved work on a named admin action**, whereas a migration backfill is an **unrequested, unattended write across every historical row** of a live database nobody here has seen (AS-09, AS-13) — which is precisely the case §7's "report the discrepancy and stop" and §4's "state your backfill" exist for, and why ARCH-1's carve-out had to be named, dated and human-approved rather than argued from first principles. So `04-01-T1` **reports and stops**: the sizing query and the two remediation `UPDATE`s ship as commented, ready-to-run header SQL and neither is executed. What is being asked of the user is authorisation, not a technical judgement. | **`04-01-T1` only, and only its commented block.** If the user says *release them*: the two `UPDATE`s are uncommented (release first, then clear the cache for appointments left with **no live junction row** — scoped by `NOT EXISTS (… is_archived = false)`, never by invoice status), the migration header states the backfill per §4, and **the constitution gains a named §7 exception dated today**, which only the user can authorise. `04-01-T2`, `04-02-T1` and `04-02-T2` are unaffected either way — deliberately, via D-27. |
| **OQ-V2** | **Does voiding need to be atomic?** `voidInvoice` will make three writes across three round trips (status, release, cache clear) with no transaction. | **No RPC.** Keep three round trips, ordered status → release → clear. See D-30: this ordering makes the worst failure degrade to *today's* behaviour, whereas releasing first would invent a double-billing window. A `SECURITY DEFINER` function is rejected under §12 and §13. | Nothing. Recorded so it is not re-proposed as a review finding against `04-02-T1`. |

### Phase 4 detail questions — defaults applied, execution proceeds

| # | Question | Default applied | Why |
|---|---|---|---|
| **DET-V1** | The edit builder's *per-invoice* junction read (`invoices/[id]/edit/page.tsx:81`) — filter it on `is_archived` too? | **No.** Only the *cross-invoice* read at `:84` is filtered. | Only `draft` invoices reach that page (`:91` → `notFound()`), and a draft's junction rows are never released, so the filter would guard a state the workflow cannot produce (constitution §14). Listed explicitly in `04-02-T2`'s out-of-scope so the asymmetry is not read as an oversight. |
...
| **DET-V3** | Migration filename. | `supabase/migrations/20260921130000_void_releases_appointments.sql`. | Next slot after `20260921120000`; `YYYYMMDDHHMMSS_description.sql` per constitution §12. |
| **DET-V4** | Must `voidInvoice` check for another live claim before clearing a released appointment's cache? | **No** — unconditional clear over the released ids. | After the release write, the partial unique index guarantees the only live claim that could have existed was this invoice's. |
| **DET-V5** | What does `voidInvoice` return if the release write fails after the status write landed? | `revalidateInvoicePaths(id)`, then `{ success: false, error: 'The invoice was voided, but its appointments were not released.' }`. The status guard is **not** loosened to make a retry possible, and `clearBilledPriceCache`'s swallowed errors stay swallowed. | The status really did change, so the screens must not lie about it. Claiming a retry path would be false — a second Void on the same invoice is refused by the guard. The swallowed cache error matches `writeBilledPriceCache` (constitution §12) and costs at worst a stale chip. |
| **DET-V6** | Should the check script gain an invariant asserting that a released junction row always belongs to a void invoice? | **No.** | True today only because `voidInvoice` is the sole writer; it would fire on any future legitimate release path, and it is not in REQ-022's acceptance. |
...
>
> **Updated 2026-09-21 (second revision): REQ-022 is now SCHEDULED as Phase 4** — the user filed it
> as GitHub issue **#18** and has scheduled it. That is OQ-R1 option **(c)**, which the planner
> recommended as "(c) when there is room". The (a) half above still stands and is exactly the
> baseline Phase 4 builds on: the `02-03` reverts are correct, the tree has `voidInvoice` writing
> only the status, and `04-02-T1` adds the release *and* the clear together rather than the clear
> alone. See D-26 through D-31 and the Phase 4 question table above.
>
...
> **Phase 5 opens no blocker either (2026-09-22).** All three of its tasks are dispatchable
> immediately, it shares no file, table or requirement with Phase 4's outstanding browser pass, and
> both of its open questions (OQ-C1, OQ-C2) carry applied defaults. Its two watch items are recorded
> as **AS-14** (no legacy row can be restored to a completion nobody recorded — the NULL fallback is
> today's behaviour, so there is nothing to backfill and nothing for the user to authorise) and
> **AS-15** (nothing writes `appointment_employees.is_archived`, and the reconcile is correct either
> way). Three real, pre-existing defects were found while establishing the phase's facts and
> **deliberately not fixed**: a scheduled appointment can be invoiced and then cancelled while still
> being billed; an employee can clock in on a cancelled appointment; and `updateAppointment`'s
> `future` scope deletes future scheduled occurrences outright, taking their clock data. All three
> are written up in `phases/05-cancel-and-edit-guards/05-CONTEXT.md §6` as candidates for their own
> issues. None is named in issue #15.
>
> **Updated 2026-09-22, after implementation and the verifier's round-2 PASS (AMD-5).** Phase 5
> still opens no blocker. Three things were added to the record and none of them changes a guard:
> - **A fourth out-of-scope finding, `05-CONTEXT.md §6.5`** — `updateAppointment` is **not
>   transactional** across its statements. If the `appointment_employees` reconcile or the
>   `future`-scope regeneration fails after the appointment `.update()` has succeeded, the status
>   and field changes stand while the crew change does not. Pre-existing, unchanged by this phase,
>   same shape as D-30's accepted non-atomicity in `voidInvoice`.
> - **An accepted limitation, KL-03 in `REQUIREMENTS.md`** — the detail page's two `'use server'`
>   wrappers discard the actions' return values (`appointments/[id]/page.tsx:157-167`), so **every**
>   error string this phase added on the Cancel / Reopen paths is invisible: both double-submit
>   refusals and both "changed while you were cancelling/reopening it" races. The admin sees a click
>   that does nothing; the write is still correctly refused. Converting that page to a Client
>   Component was put out of scope by `05-CONTEXT.md §5` and `05-02-PLAN.md` T2, so this was
>   **deliberately not fixed**. The edit page is unaffected — `appointment-form.tsx` renders
>   `state.error`, so `updateAppointment`'s refusals, REQ-028's included, are displayed.
> - **OQ-C3**, above — the one item that genuinely needs the user, and the only reason to hold the
>   phase open beyond the pending browser pass.

> **Phase 4 opens no blocker.** All four of its tasks are dispatchable immediately. OQ-V1 and OQ-V2
> are open questions with applied defaults, not blockers: `04-01-T1` ships the OQ-V1 remediation as
> commented header SQL and runs it never, so a "release them too" answer arriving later is two
> `UPDATE`s plus a named constitution §7 exception, not a replan. The two Phase 4 risks worth
> watching are recorded as **AS-12** (the first-ever `UPDATE` against `invoice_appointments`,
> descendant of B-01) and **AS-13** (the unknown count of already-voided invoices, descendant of
> AS-09), both in the Assumptions table.
